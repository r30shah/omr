...
{
   fprintf(stderr,"RAHIL: Called isRunningInContainer\n");
   if (OMRPORT_PROCESS_IN_CONTAINER_UNINITIALIZED == PPG_processInContainerState) {
		omrthread_monitor_enter(cgroupMonitor);
      fprintf(stderr,"RAHIL: omrsysinfo.c:6665\n");
		if (OMRPORT_PROCESS_IN_CONTAINER_UNINITIALIZED == PPG_processInContainerState) {
			FILE *cgroupFile = NULL;
			int32_t rc = 0;
			 ...
			 */
         fprintf(stderr,"RAHIL: omrsysinfo.c:6676\n");
			if ((0 == access("/.dockerenv", F_OK)) || (0 == access("/run/.containerenv", F_OK))) {
            fprintf(stderr,"RAHIL: omrsysinfo.c:6678\n");
				inContainer = TRUE;
