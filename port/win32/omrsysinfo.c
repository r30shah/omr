...
getWindowsOSVersion(struct OMRPortLibrary *portLibrary, OSVERSIONINFOEXW *versionInfo)
{
	...
	memset(versionInfo, 0, sizeof(OSVERSIONINFOEXW));
...
#pragma warning(suppress : 4996)
	...
	if ((0 != pathLen) && (pathLen < MAX_PATH)) {
		DWORD infoSize = GetFileVersionInfoSizeW(kernel32Path, NULL);

		if (0 != infoSize) {
			void *versionBuffer = portLibrary->mem_allocate_memory(
					portLibrary, infoSize,
					OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
			if (NULL != versionBuffer) {
				if (GetFileVersionInfoW(kernel32Path, 0, infoSize, versionBuffer)) {
					VS_FIXEDFILEINFO *fixedInfo = NULL;
					UINT fixedInfoLen = 0;
					if (VerQueryValueW(versionBuffer, L"\\", (LPVOID *)&fixedInfo, &fixedInfoLen)
						&& (NULL != fixedInfo)
...
omrsysinfo_get_OS_version(struct OMRPortLibrary *portLibrary)
{
	...
	if (NULL == PPG_si_osVersion) {
		OSVERSIONINFOEXW versionInfo;
		BOOL isVersionInfoValid = getWindowsOSVersion(portLibrary, &versionInfo);
		if (isVersionInfoValid && (10 <= versionInfo.dwMajorVersion)) {
