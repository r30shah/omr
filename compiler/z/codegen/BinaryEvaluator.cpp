    ...
    TR::Compilation *comp)
{
    ...
    logprintf(trace, log, "in find at %p, looking for %p for %d\n", prev, useA, searchLimit);
    while (searchLimit-- && curInsn) {
        logprintf(trace, log, "Looking at %p %d\n", curInsn, searchLimit);
        ...
        if (op.isAdmin() || op.isLabel()) {
            ...
            logprintf(trace, log, "Def found:%p\n", curInsn);
            return curInsn;
        }
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCodeValue() == TR::lshl) {
            if (firstChild->getOpCodeValue() == TR::i2l && firstChild->getReferenceCount() == 1
                ...
                trgReg = TR::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(firstChild, cg, value,
...
TR::Register *OMR::Z::TreeEvaluator::dualMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDualCyclic() && node->getChild(2)->getReferenceCount() == 1) {
        ...
        TR::Node *lumulhNode;
        if (!needsUnsignedHighMulOnly) {
            logprintf(comp->getOption(TR_TraceCG), comp->log(), "Found lmul/lumulh for node = %p\n", node);
