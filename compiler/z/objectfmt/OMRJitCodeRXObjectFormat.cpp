...
TR::Instruction *
OMR::Z::JitCodeRXObjectFormat::emitGlobalFunctionCall(TR::GlobalFunctionCallData &data)
   {
...
uint8_t *
OMR::Z::JitCodeRXObjectFormat::encodeGlobalFunctionCall(TR::GlobalFunctionCallData &data)
   {
   TR::SymbolReference *callSymRef = data.globalMethodSymRef;
   TR::CodeGenerator *cg = data.cg;
...
OMR::Z::JitCodeRXObjectFormat::printEncodedFunctionCall(TR::FILE *pOutFile, TR::FunctionCallData &data)
   ...
   uint8_t *ccFunctionCallDataAddress = reinterpret_cast<uint8_t*>(*(reinterpret_cast<int32_t*>(bufferPos + sizeof(int16_t))) * 2) + bufferPos;
   debug->printPrefix(pOutFile, NULL, bufferPos, 6);
   trfprintf(pOutFile, "LGRL \tGPR14, <%p>\t# Target Address = <%p>.",
                        ccFunctionCallDataAddress, *ccFunctionCallDataAddress);
   bufferPos += 6;
