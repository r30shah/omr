...
protected:
    PatchNOPedGuardSite(TR_PersistentMemory *pm, uintptr_t key, TR_RuntimeAssumptionKind kind, uint8_t *location,
        uint8_t *destination)
...
protected:
    PatchMultipleNOPedGuardSites(TR_PersistentMemory *pm, uintptr_t key, TR_RuntimeAssumptionKind kind,
        PatchSites *sites)
        : OMR::LocationRedirectRuntimeAssumption(pm, key)
        , _patchSites(sites)
    {}
